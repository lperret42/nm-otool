...
/*   Updated: 2018/04/23 21:33:17 by lperret          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int					handle_64(char *p, t_options opts)
{
	...
	//if (!(header = (struct mach_header_64 *)check_addr(p, sizeof(*header))))
	if (check_addr((void**)&header, p, sizeof(*header)) != 0)
		return (FORMAT_ERROR);
	//if (!(lc = (struct load_command *)(check_addr(p + sizeof(*header), sizeof(*lc)))))
	if (check_addr((void**)&lc, p + sizeof(*header), sizeof(*lc)) != 0)
		return (FORMAT_ERROR);
	//if (!(sect_names = get_sections_name(lc, get_nb_sects(lc, header->ncmds))))
	if (!(sect_names = get_section_names_64(lc)))
		return (FORMAT_ERROR);
	i = -1;
