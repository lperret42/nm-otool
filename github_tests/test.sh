...
if [ $TESTNM != 0 ]; then
	...
	while IFS='' read -r -d '' filename; do
		[ $? == 1 ] && exit 0;
		FILE1=$filename
		nm $FILE1 > their ; ../nm/ft_nm $FILE1 > mine
	...
	nm /bin/bash > their ; ../nm/ft_nm /bin/bash > mine
	COUNT=$(diff their mine | wc -l)
	if [ $COUNT != 0 ]; then
	...
	[ $? == 1 ] && exit 0;
	nm /bin/sh > their ; ../nm/ft_nm /bin/sh > mine
	COUNT=$(diff their mine | wc -l)
	if [ $COUNT != 0 ]; then
	...
	[ $? == 1 ] && exit 0;
	nm /bin/sync > their ; ../nm/ft_nm /bin/sync > mine
	COUNT=$(diff their mine | wc -l)
	if [ $COUNT != 0 ]; then
	...
	while IFS='' read -r -d '' filename; do
		...
		if [ "$FILE1" != "" -a "$FILE1" != "/usr/lib/libnetsnmp.5.2.1.dylib" -a "$FILE1" != "/usr/lib/libsqlite3.dylib" ]; then
			...
			../nm/ft_nm  $FILE1 2>/dev/null > mine
	...
	while IFS='' read -r -d '' filename; do
		...
		if [ "$FILE1" != "/usr/lib/libnetsnmp.5.2.1.dylib" ]; then
			...
			../nm/ft_nm $FILE1  2> mine_err > mine
			COUNT=$(cat their mine | sort | uniq -u | wc -l)
